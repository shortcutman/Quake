...
double loopTime, oldtime, newtime;

void Sys_Looper(void)
{
    extern int vcrFile;
    ...
    {   // play vcrfiles at max speed
        if (loopTime < sys_ticrate.value && (vcrFile == -1 || recording) )
        {
            usleep(1);
            return;       // not time to run a server only tic yet
        }
        loopTime = sys_ticrate.value;
    }

    if (loopTime > sys_ticrate.value*2)
        oldtime = newtime;
    else
        oldtime += loopTime;
